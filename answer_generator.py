...
class AnswerGenerator:
    ...
    def _sem_hint(sem: str) -> str:
        ...
        if sem == "ORGANIZATION":
            return "The expected answer is likely an organization, company, school, team, agency, or institution."
        if sem == "TIME":
            return "The expected answer is likely a date, year, period, or time expression."
        if sem == "NUMBER":
            return "The expected answer is likely a numeric value such as a count, rank, age, amount, or measurement."
        if sem == "WORK":
            return "The expected answer is likely a named work such as a book, film, paper, song, software, or law text."
        if sem == "EVENT":
            return "The expected answer is likely a named event such as an award, war, election, competition, conference, or case."
        if sem == "CONCEPT":
            return "The expected answer is likely a concept, method, technology, category, or abstract term."
        if sem == "BOOLEAN":
            return "The expected answer is yes/no or true/false."
        if sem == "DESCRIPTION":
            return "The expected answer is explanatory or descriptive, such as a reason, process, or difference."
        return "No type-specific hint is needed for this question."

    @staticmethod
    def _topo_hint(topo: str) -> str:
        topo = (topo or "BRIDGE").strip().upper()
        ...
        if topo == "DIRECT":
            return "Note: this question may be answerable from one direct evidence path."
    ...
    def build_rag_prompt(
        ...
        parts.append("\n# Intent")
        parts.append(f"- Reasoning type: {intent_topo} ({self._topo_hint(intent_topo)})")
        parts.append(f"- Expected answer type: {intent_sem} ({self._sem_hint(intent_sem)})")
